...
```python
# config/settings/base.py  (dev/prod override the auth-class list to add cookie auth)
REST_FRAMEWORK = {
    "DEFAULT_AUTHENTICATION_CLASSES": [
        "rest_framework_simplejwt.authentication.JWTAuthentication",      # 1. Bearer header
        "accounts.services.auth.CustomJWTCookieAuthentication",          # 2. cookie fallback
        "rest_framework.authentication.SessionAuthentication",           # 3. admin
    ],
    ...
    "DEFAULT_THROTTLE_RATES": {        # development.py
        "anon": "100/minute",
        "user": "200/minute",
...
> wins. The cookie class only engages when there's no header (Swagger/admin browser use).
> Keep `IsAuthenticated` as the default permission so endpoints are **closed by default** —
> opt *into* `AllowAny` explicitly on public auth endpoints.
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    throttle_scope = "login"          # uses the "login" throttle rate
...
> The throttle scope `"login"` ties this endpoint to `DEFAULT_THROTTLE_RATES["login"]`
> (30/min dev, **10/min prod**) — your first line of defense against credential stuffing.
